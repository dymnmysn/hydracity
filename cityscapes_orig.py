...
def __getitem__(self, idx, scale_factor=0.5):
    """
    ...
    disparity_np = np.array(disparity_image).astype(np.float32) / 256.0
...
class CityScapes_rgbd(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            dict: A dictionary with 'rgbd' and 'label' keys.
        """
        rgb_file, disparity_file, label_file = self.samples[idx]
        ...
        rgb_tensor = F.to_tensor(rgb_image)  # Shape: (3, H, W)
        disparity_np = np.array(disparity_image).astype(np.float32) / 256.0  # Convert to float
        disparity_tensor = torch.from_numpy(disparity_np).unsqueeze(0)  # Shape: (1, H, W)
        label_np = np.array(label_image, dtype=np.int64)  # Shape: (H, W)

        # Remap the label IDs to the 19 training classes
        label_mapped = np.vectorize(CITYSCAPES_LABEL_MAP.get)(label_np)
        label_tensor = torch.from_numpy(label_mapped).long()  # Shape: (H, W)

        # Combine RGB and disparity into an RGB-D tensor
        rgbd_tensor = torch.cat([rgb_tensor, disparity_tensor], dim=0)  # Shape: (4, H, W)

        if self.transform:
            rgbd_tensor, label_tensor = self.transform(rgbd_tensor, label_tensor)

        return {"rgbd": rgbd_tensor, "label": label_tensor}
...
class CityScapes(Dataset): #YUVD dataset
    ...
    def __getitem__(self, idx, scale_factor=0.25):
        ...
        yuvd_downscaled = downscale_exclude_invalid_multichannel(yuvd_np, scale_factor)
        label_downscaled = downscale_exclude_invalid_multichannel(
            label_np[np.newaxis, ...], scale_factor
        )[0]  # Remove the channel dimension
    ...
    def __getitem__(self, idx):
        ...
        # Convert RGB to YUV
        rgb_np = np.array(rgb_image)  # Convert to NumPy array
        yuv_np = cv2.cvtColor(rgb_np, cv2.COLOR_RGB2YUV)  # Convert RGB to YUV
        yuv_tensor = F.to_tensor(yuv_np).float()  # Convert to tensor
        ...
        # Combine YUV and disparity into a tensor
        yuvd_tensor = torch.cat([yuv_tensor, disparity_tensor], dim=0)  # Shape: (4, H, W)

        if self.transform:
            yuvd_tensor, label_tensor = self.transform(yuvd_tensor, label_tensor)

        return {"rgbd": yuvd_tensor, "label": label_tensor}
    """